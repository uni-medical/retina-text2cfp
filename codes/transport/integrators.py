...
class ode:
    ...
    def sample(self, x, model, **model_kwargs):
        ...
        # print('--->> before: inside ode x:', x.shape)
        
        device = x[0].device if isinstance(x, tuple) else x.device
        
        # print('--->> before: inside ode x[0]:', x[0].shape)
        
        
        def _fn(t, x):
            t = th.ones(x[0].size(0)).to(device) * t if isinstance(x, tuple) else th.ones(x.size(0)).to(device) * t
            # print('--->> inside: _fn x[0].size(0): ', x[0].size(0))
            # print('--->> inside: _fn  t:', t.shape)
            model_output = self.drift(x, t, model, **model_kwargs)
            return model_output

        t = self.t.to(device)
        # print('--->> before: inside ode t:', t.shape)
